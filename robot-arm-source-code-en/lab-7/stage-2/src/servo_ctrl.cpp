// ============================================================
// LAB 7 - STAGE 2: SERVO CONTROL IMPLEMENTATION
// ============================================================
...
// Mang Save goc hien Load cua 6 Joint (bien toan cuc)
int currentAngles[6];
...
// ============================================================
// servo_init() - Initialize PCA9685 va dua All servo Return to Home
// ============================================================
void servo_init() {
  ...
  Serial.println("[Servo] All Joint da Return to Home");
}

// ============================================================
// servo_move() - Di chuyen 1 Joint den goc chi dinh
...
int servo_move(int joint, int angle) {
  ...
  // Gioi han goc trong pham vi hop le cua Joint
  ...
  Serial.printf("[Servo] Joint %d (%s) → %d do\n",
                joint, JOINTS[joint].name, angle);
  return angle;
}

// ============================================================
// servo_home() - Dua Joint ve vi tri mac dinh
// joint = -1 → All Joint; joint = 0..5 → 1 Joint
// ============================================================
void servo_home(int joint) {
  if (joint == -1) {
    ...
    Serial.println("[Servo] All Joint Return to Home");
    ...
    Serial.printf("[Servo] Joint %d Return to Home (%d do)\n",
                  joint, JOINTS[joint].homeAngle);
  }
}

// ============================================================
// servo_moveAll() - Move simultaneously 6 Joint
// Input: mang angles[6] chua goc moi cho tung Joint
// ============================================================
void servo_moveAll(int angles[6]) {
  ...
  Serial.println("[Servo] MoveAll Completed");
}

// ============================================================
// servo_getStatusString() - Tao chuoi trang thai "a0,a1,...,a5"
// Dung cho lenh T (Get Status) return client
// ============================================================
