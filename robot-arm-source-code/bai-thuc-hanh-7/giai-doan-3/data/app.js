// ============================================================
// BAI THUC HANH 7 - GIAI DOAN 3: WEBSOCKET CLIENT (app.js)
// Diem nang cap so voi giai doan 2:
//   1. Giao thuc JSON (thay vi plain-text)
//   2. Auto-reconnect voi exponential backoff
//   3. Pose Save/Load qua REST API (/api/pose/save, /api/pose/load)
//
// Giao thuc JSON (so sanh):
//   Giai doan 2:  "M 0 90"           → Giai doan 3: {"cmd":"M","joint":0,"angle":90}
//   Giai doan 2:  "H"                 → Giai doan 3: {"cmd":"H"}
//   Giai doan 2:  "T"                 → Giai doan 3: {"cmd":"T"}
//   Giai doan 2:  "STA:90,70,..."     → Giai doan 3: {"ok":true,"angles":[90,70,...]}
// ============================================================
...
let reconnectDelay = 1000;    // Bat dau 1 giay, tang dan (exponential backoff)
let reconnectTimer = null;

// === KHOI TAO GIAO DIEN ===
function buildUI() {
  // 6 Servo Cards (giong giai doan 2)
  ...
  // 5 Pose Slots (tinh nang moi giai doan 3)
  ...
  poseSlots.innerHTML = Array.from({length: 5}, (_, i) => `
    <div class="pose-slot">
      ...
      <button class="btn-pose-save" onclick="poseSave(${i})">&#x1F4BE; Luu</button>
      <button class="btn-pose-load" onclick="poseLoad(${i})">&#x25B6; Tai</button>
    </div>
  `).join('');
}
...
function connect() {
  ...
  ws.onopen = () => {
    setStatus(true);
    reconnectDelay = 1000;  // Reset delay khi ket noi thanh cong
    log('WebSocket: Ket noi thanh cong!', 'info');
    sendJson({ cmd: 'T' });  // Lay trang thai hien tai
  };

  ws.onclose = () => {
    setStatus(false);
    log(`Mat ket noi. Thu lai sau ${reconnectDelay/1000}s...`, 'error');
...
function handleResponse(data) {
  if (Array.isArray(data.angles) && data.angles.length === 6) {
    // Cap nhat UI khi nhan duoc trang thai angles
...
function onSlider(jointId, value) {
  ...
  debounces[jointId] = setTimeout(() => {
    // Giao thuc JSON (giai doan 3) thay vi plain-text (giai doan 2)
...
// Luu tu the hien tai vao slot (qua REST API)
async function poseSave(slot) {
  try {
    ...
    if (data.ok) {
      log(`[Pose] Da luu Pose ${slot}`, 'info');
    } else {
      log(`[Pose] Loi luu: ${data.error}`, 'error');
    }
  } catch (e) {
    log(`[Pose] Khong the ket noi API: ${e}`, 'error');
  }
}

// Tai tu the tu slot va cap nhat UI
async function poseLoad(slot) {
  try {
    ...
    if (data.ok && data.angles) {
      ...
      log(`[Pose] Da tai Pose ${slot}: ${data.angles.join(', ')}`, 'info');
    } else {
      log(`[Pose] Slot ${slot} chua co du lieu`, 'error');
    }
  } catch (e) {
    log(`[Pose] Khong the ket noi API: ${e}`, 'error');
  }
