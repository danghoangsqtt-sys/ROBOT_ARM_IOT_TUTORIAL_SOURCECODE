// ============================================================
// BAI THUC HANH 7 - GIAI DOAN 2: WEBSOCKET HANDLER
// Mo ta: Xu ly cac su kien WebSocket va parse lenh plain-text.
//        Giao thuc: giong BTH 4-5 Serial (M, A, H, T, S, W)
// ============================================================

#include "ws_handler.h"
#include "servo_ctrl.h"

// ============================================================
// _parseCommand() - Ham noi bo: xu ly 1 chuoi lenh
// Giao thuc (giong BTH 4-5 Serial Protocol):
//   "M <j> <angle>"       → Di chuyen khop j
//   "A <a0> <a1>...<a5>"  → Di chuyen dong thoi 6 khop
//   "H" / "H <j>"         → Home tat ca hoac 1 khop
//   "T"                   → Tra ve trang thai hien tai
//   "S <j> <speed>"       → Dat toc do (ghi nhan, chua ap dung)
// ============================================================
static void _parseCommand(AsyncWebSocketClient* client, const String& cmd) {
  Serial.println("[WS] Nhan: " + cmd);

  if (cmd.startsWith("M ")) {
    // --- M <joint> <angle>: Di chuyen 1 khop ---
    int sp1 = cmd.indexOf(' ', 2);
    if (sp1 > 0) {
      int joint = cmd.substring(2, sp1).toInt();
      int angle = cmd.substring(sp1 + 1).toInt();
      int actual = servo_move(joint, angle);
      if (actual >= 0) {
        client->text("OK");
      } else {
        client->text("ERR: Khop khong hop le");
      }
    }

  } else if (cmd.startsWith("A ")) {
    // --- A <a0> <a1> <a2> <a3> <a4> <a5>: Di chuyen dong thoi ---
    int angles[6] = {0};
    String rest   = cmd.substring(2);
    int count     = 0;
    int start     = 0;
    // Parse 6 gia tri so nguyen ngan cach nhau boi dau cach
    while (count < 6) {
      int sp = rest.indexOf(' ', start);
      String token = (sp == -1) ? rest.substring(start)
                                : rest.substring(start, sp);
      angles[count++] = token.toInt();
      if (sp == -1) break;
      start = sp + 1;
    }
    if (count == 6) {
      servo_moveAll(angles);
      client->text("OK");
    } else {
      client->text("ERR: Can 6 gia tri goc");
    }

  } else if (cmd == "H") {
    // --- H: Home tat ca khop ---
    servo_home(-1);
    client->text("OK");

  } else if (cmd.startsWith("H ")) {
    // --- H <joint>: Home 1 khop ---
    int joint = cmd.substring(2).toInt();
    servo_home(joint);
    client->text("OK");

  } else if (cmd == "T") {
    // --- T: Tra ve trang thai hien tai ---
    client->text(servo_getStatusString());

  } else if (cmd.startsWith("S ")) {
    // --- S <joint> <speed>: Ghi nhan toc do (giai doan 2 chua su dung) ---
    // Tinh nang nay se duoc ap dung day du o giai doan 3
    client->text("OK");

  } else {
    client->text("ERR: Lenh khong xac dinh: " + cmd);
  }
}

// ============================================================
// ws_onEvent() - Xu ly cac su kien WebSocket
// ============================================================
void ws_onEvent(AsyncWebSocket* server, AsyncWebSocketClient* client,
                AwsEventType type, void* arg, uint8_t* data, size_t len) {
  switch (type) {
    case WS_EVT_CONNECT:
      Serial.printf("[WS] Client %u ket noi (IP: %s)\n",
                    client->id(), client->remoteIP().toString().c_str());
      // Gui trang thai ban dau khi client vua ket noi
      client->text(servo_getStatusString());
      break;

    case WS_EVT_DISCONNECT:
      Serial.printf("[WS] Client %u ngat ket noi\n", client->id());
      break;

    case WS_EVT_DATA: {
      AwsFrameInfo* info = (AwsFrameInfo*)arg;
      // Chi xu ly frame TEXT hoan chinh (khong phan manh)
      if (info->final && info->index == 0 && info->len == len
          && info->opcode == WS_TEXT) {
        String cmd = String((char*)data, len);
        cmd.trim();
        if (cmd.length() > 0) {
          _parseCommand(client, cmd);
        }
      }
      break;
    }

    case WS_EVT_ERROR:
      Serial.printf("[WS] Loi client %u\n", client->id());
      break;

    default:
      break;
  }
}

// ============================================================
// ws_setup() - Dang ky callback vao doi tuong WebSocket
// ============================================================
void ws_setup(AsyncWebSocket& ws) {
  ws.onEvent(ws_onEvent);
}
