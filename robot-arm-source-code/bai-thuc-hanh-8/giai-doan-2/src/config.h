...
// ── WiFi & Web Server ────────────────────────────────────────
#define CFG_WIFI_SSID       "lab_wifi"
#define CFG_WIFI_PASSWORD   "lab_password"
#define CFG_WIFI_TIMEOUT_MS 10000UL     // connection timeout (ms)
...
//  angle_PW = PW_min + (PW_max - PW_min) * (theta - theta_min) / (theta_max - theta_min)
//  PW = Pulse Width
...
static const ServoDef SERVO_TABLE[CFG_NUM_SERVOS] = {
    ...
    {  1,   0,  180,   150,  500,  2500,    1 },  // J1 – shoulder
    {  2,   0,  150,   90,   500,  1833,    1 },  // J2 – elbow
    {  3,   0,  180,   90,   500,  2500,    1 },  // J3 – wrist pitch
    {  4,   0,  180,   90,   500,  2500,    1 },  // J4 – wrist roll
    {  5,   60, 120,   90,   1333, 1667,    1 },  // J5 – gripper (0=open, 90=closed)
};

// uint16_t angleToPulseWidth(float angle) {
//   float minAngle = 0.0;
//   float maxAngle = 180.0;
//   float minPulse = 500.0; // or 500 for extended range
//   float maxPulse = 2500.0; // or 2500 for extended range
  
//   // Constrain the input angle to safe limits
//   angle = constrain(angle, minAngle, maxAngle);
  
//   // Calculate pulse width
//   return minPulse + ((angle - minAngle) * (maxPulse - minPulse)) / (maxAngle - minAngle);
// }