...
CmdParser::CmdParser(ServoCtrl& sc)
    : _sc(sc), _len(0), _waitDone(false), _responseCapture(nullptr) {}
...
String CmdParser::execute(const char* cmdStr) {
    char oldBuf[CFG_RX_BUF];
    memcpy(oldBuf, _buf, CFG_RX_BUF);
    uint8_t oldLen = _len;

    strncpy(_buf, cmdStr, CFG_RX_BUF - 1);
    _buf[CFG_RX_BUF - 1] = '\0';
    _len = strlen(_buf);

    String resp = "";
    _responseCapture = &resp;

    process();

    _responseCapture = nullptr;

    memcpy(_buf, oldBuf, CFG_RX_BUF);
    _len = oldLen;
...
void CmdParser::respond(const char* msg) {
    if (_responseCapture) {
        if (_responseCapture->length() > 0) _responseCapture->concat("\n");
        _responseCapture->concat(msg);
    } else {
        Serial.println(msg);
    }
}
...
void CmdParser::respondErr(const char* reason) {
    char out[32];
    snprintf(out, sizeof(out), "ERR:%s", reason);
    respond(out);
}
